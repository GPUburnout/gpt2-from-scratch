...
def load_model(model_dir='models/tiny_lm_v1'):
    ...
    # Load config first to determine tokenizer type
    config_path = os.path.join(model_dir, 'config.json')
    with open(config_path, 'r') as f:
        config = json.load(f)

    # Load tokenizer (dynamic based on config)
    ...
    if tokenizer_type == 'bpe':
        ...
        # Check for bpe_tokenizer.json naming
        bpe_path = os.path.join(model_dir, 'bpe_tokenizer.json')
